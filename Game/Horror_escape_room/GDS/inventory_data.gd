...
func use_slot_data(index: int) -> void:
	...
	if slot_data.item_data.name == "flaslight":
		slot_datas[index] = null
	
	#Ha Battery akkor töltse újra az lámpát
	if slot_data.item_data.name == "Battery":
		Global.battery_used = true
		Global.EG = 10.0
	
	#csökkentse az item db számát
